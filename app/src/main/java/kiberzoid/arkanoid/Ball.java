...
public class Ball {
    ...
    private SoundPool soundPlatform;
    int idF;
    ...
    public Ball(float xPos,float yPos, int radius,int xSpeed, int ySpeed){
        ...
        this.radius = radius;
        this.fail = false;
       // soundPlatform = new SoundPool.Builder().build();
        //soundPlatform = new SoundPool(3, AudioManager.STREAM_MUSIC, 0);
        //idF = soundPlatform.load("background.mp3",1);
    ...
    public void update(float width, float height, Platform platform, ArrayList<Block> blocks, SoundPool sp, int p_otskok_id, int fail_id){
        boolean x_need_return = false;
        boolean y_need_return = false;
        if (blocks.isEmpty()) this.fail=true;
